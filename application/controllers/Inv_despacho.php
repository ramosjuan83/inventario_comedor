...
class Inv_despacho extends Controlador_padre {
    ...
    public function listar($memoria = 'false'){
        ...
        if($logged == TRUE && $id_conf_roles_es_5 == true){ 
                ...
                if($memoria == 'limpiar'){
                    $this->session->unset_userdata('inv_despacho_b_texto');
                }
                
                if (	isset ($_POST['b_texto'])	){
                        $b_texto = $_POST['b_texto'];
                        $s_busquedad = array(
                                'inv_despacho_b_texto' => $b_texto
                        );
                        $this->session->set_userdata($s_busquedad);
                }else{
                        $b_texto = "";
                        if( strlen($this->session->userdata('inv_despacho_b_texto')) > 0  ){
                            $b_texto = $this->session->userdata('inv_despacho_b_texto');
                        }
                }
                $b_texto = str_replace("'", "", $b_texto);
                
                if($memoria == 'cargo_ultima_pagina'){
                        $pagina_actual = $this->session->userdata('inv_despacho_ultima_pagina');    
                        ...
                        $s_lista = array(
                            'inv_despacho_ultima_pagina' => $pagina_actual
                        );
                ...
                $config['base_url'] = base_url().'index.php/inv_despacho/listar/pagina'; // parametro base de la aplicación, si tenemos un .htaccess nos evitamos el index.php
                $total_rows = $this->Inv_despacho_model->inv_despacho_num_reg($b_texto);
                ...
                $dat_list = $this->Inv_despacho_model->inv_despacho_buscar($b_texto, $config['per_page'],$segmento);
//echo "<br>dat_list *<pre>"; print_r($dat_list); echo "</pre>*";                
                
                if($dat_list != false){
                    for($i = 0; $i < count($dat_list); $i++){
                        // $despacho_id = $dat_list[$i]->id;  //echo "gerencias_id *".$gerencias_id."*";
                        // unset($matriz_comensales);                        
                        // $matriz_personal_ivic = $this->Personal_ivic_model->personal_ivic_buscar_9($despacho_id);
...
//echo "<br>dat_list *<pre>"; print_r($dat_list); echo "</pre>*";
                ...
                $this->load->view('inv_despacho/v_inv_despacho_listar', $dat_list);
    ...
    public function agregar(){
        ...
        if($logged == TRUE && $id_conf_roles_es_5 == true){
            ...
            $this->load->view('plantilla/header', $data);
            $this->load->view('plantilla/menu');
            $this->load->view('inv_despacho/v_inv_despacho_frm', $data);
            $this->load->view('plantilla/footer');                
        }else{
            $this->session->sess_destroy('username');
            $data['ruta_llamados_head'] = "plantilla/llamados_head/llamados_head_basicos2.php";
    ...
    public function agregar_guardar(){
        ...
        if($logged == TRUE){            
            ...
            if($valido == true){
                    $id = $this->Inv_despacho_model->inv_despacho_insertar($nombre,$id_articulo,$fecha,$id_almacen,$cantidad);
                    if ($id > 0){
                        $mensaje = "Inserto la despacho con id ".$id;
                        $mensaje_2 = "Inserto la despacho ".$nombre;
                        $s_mensaje = array(
                                'inv_despacho_mensaje_tipo'         => 'alert-success',
                                'inv_despacho_mensaje_contenido'    => 'Registros insertado exitosamente, '.$mensaje_2
                        );
                        $this->session->set_userdata($s_mensaje);
                        $this->insertar_bitacora_2("inv_despacho", $mensaje, $mensaje_2);
                        redirect('inv_despacho/listar/cargo_ultima_pagina');
                    }else{
                        redirect('inv_despacho/agregar');
                    }                    
            }else{
                $mensaje_2 = "El despacho ";
                $s_mensaje = array(
                    'inv_despacho_mensaje_tipo'         => 'alert-danger',
                    'inv_despacho_mensaje_contenido'    => 'La cantidad es superior a la disponibilidad , '." ".$disponible." ".$mensaje_2
                );
                $this->session->set_userdata($s_mensaje);
                redirect('Inv_despacho/agregar');
            }
    ...
    public function editar($id){
        ...
        if($logged == TRUE && $id_conf_roles_es_5 == true){
            ...
            $fila_reg = $this->Inv_despacho_model->inv_despacho_buscar_2($id);   //echo "<pre>"; print_r($fila_reg); echo "</pre>";
            $data['fila_registro'] = $fila_reg[0]; 
            if(strlen($fila_reg[0]->fecha_egreso) > 2){
                $fila_reg[0]->fecha_egreso = $this->ordena_fecha_4($fila_reg[0]->fecha_egreso);    
            }
            $data['oper'] = "editar";
    ...
    public function editar_guardar(){
        ...
        if($logged == TRUE){
            ...
            if($valido == true){
                $oper_realizada = $this->Inv_despacho_model->inv_despacho_editar($id,$id_articulo,$fecha,$id_almacen,$cantidad,$nombre);
                //echo "oper_realizada *".$oper_realizada."*";
                if ($oper_realizada){
                    $mensaje = "Actualizó el despacho con id ".$id;
                    $mensaje_2 = "Actualizó el despacho ".$nombre;
                    $s_mensaje = array(
                            'inv_despacho_mensaje_tipo'  => 'alert-success',
                            'inv_despacho_mensaje_contenido'    => 'Registros actualizado exitosamente, '.$mensaje_2
                    );
                    $this->session->set_userdata($s_mensaje);
                    $this->insertar_bitacora_2("inv_despacho", $mensaje, $mensaje_2);
                ...
                $this->session->set_userdata($s_mensaje);
                redirect('Inv_despacho/editar/'.$id);
            }
        }else{
            $this->session->sess_destroy('username');
            $data['ruta_llamados_head'] = "plantilla/llamados_head/llamados_head_basicos2.php";
    ...
    public function eliminar($id){
            ...
            if($logged == TRUE && $id_conf_roles_es_5 == true){
                ...
                $matriz_despacho = $this->Inv_despacho_model->inv_despacho_buscar_2($id);
                $despacho_nombre = $matriz_despacho[0]->nombres;
                ...
                if( $valido == true ){
                    $oper_realizada = $this->Inv_despacho_model->inv_despacho_eliminar($id);
                    if ($oper_realizada){
                            $mensaje = "Eliminó el despacho con id ".$id;
                            $mensaje_2 = "Eliminó el despacho ".$despacho_nombre;
                            $s_mensaje = array(
                                    'inv_despacho_mensaje_tipo'         => 'alert-success',
                                    'inv_despacho_mensaje_contenido'    => 'Registros eliminado exitosamente, '.$mensaje_2
                            );
                            $this->session->set_userdata($s_mensaje);
                            $this->insertar_bitacora_2("inv_despacho", $mensaje, $mensaje_2);
                    }                
    ...
    public function ver_pdf_lista($b_texto){
        ...
        if($logged == TRUE){
            ...
            $matriz_despacho = $this->Inv_despacho_model->inv_despacho_buscar($b_texto, "", ""); 

            $data['matriz_despacho'] = $matriz_despacho;   
            
            $this->load->view('inv_despacho/reporte_pdf_lista', $data); 
        }        
    }  
    ...
    public function ver_excel_lista($b_texto){
        ...
        if($logged == TRUE){
            ...
            $this->load->view('inv_despacho/reporte_excel_lista.php', $data);
    ...
    public function get_inv_despacho_buscar_5(){
            $nombre     = $this->input->post('nombre');
            $id         = $this->input->post('id');
            $resultados     = $this->Inv_despacho_model->inv_despacho_buscar_5($nombre, $id); //echo "resultados *"; print_r($resultados); echo "*";
            echo json_encode($resultados);
    }

    public function get_inv_despacho_buscar_6(){
            $nombre     = $this->input->post('nombre');
            $resultados = $this->Inv_despacho_model->inv_despacho_buscar_6($nombre); //echo "resultados *"; print_r($resultados); echo "*";
            echo json_encode($resultados);
    }  
