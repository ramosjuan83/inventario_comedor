...
class Inv_almacen extends Controlador_padre {
    ...
    public function listar($memoria = 'false'){
        ...
        if($logged == TRUE && $id_conf_roles_es_5 == true){ 
                ...
                if($memoria == 'limpiar'){
                    $this->session->unset_userdata('inv_almacen_b_texto');
                }
                
                if (	isset ($_POST['b_texto'])	){
                        $b_texto = $_POST['b_texto'];
                        $s_busquedad = array(
                                'inv_almacen_b_texto' => $b_texto
                        );
                        $this->session->set_userdata($s_busquedad);
                }else{
                        $b_texto = "";
                        if( strlen($this->session->userdata('inv_almacen_b_texto')) > 0  ){
                            $b_texto = $this->session->userdata('inv_almacen_b_texto');
                        }
                }
                $b_texto = str_replace("'", "", $b_texto);
                
                if($memoria == 'cargo_ultima_pagina'){
                        $pagina_actual = $this->session->userdata('inv_almacen_ultima_pagina');    
                        ...
                        $s_lista = array(
                            'inv_almacen_ultima_pagina' => $pagina_actual
                        );
                ...
                $config['base_url'] = base_url().'index.php/inv_almacen/listar/pagina'; // parametro base de la aplicación, si tenemos un .htaccess nos evitamos el index.php
                $total_rows = $this->Inv_almacen_model->inv_almacen_num_reg($b_texto);
                ...
                $dat_list = $this->Inv_almacen_model->inv_almacen_buscar($b_texto, $config['per_page'],$segmento);
//echo "<br>dat_list *<pre>"; print_r($dat_list); echo "</pre>*";                
                
                if($dat_list != false){
                    for($i = 0; $i < count($dat_list); $i++){
                        // $almacen_id = $dat_list[$i]->id;  //echo "gerencias_id *".$gerencias_id."*";
                        // unset($matriz_comensales);                        
                        // $matriz_personal_ivic = $this->Personal_ivic_model->personal_ivic_buscar_9($almacen_id);
...
//echo "<br>dat_list *<pre>"; print_r($dat_list); echo "</pre>*";
                ...
                $this->load->view('inv_almacen/v_inv_almacen_listar', $dat_list);
    ...
    public function agregar(){
        ...
        if($logged == TRUE && $id_conf_roles_es_5 == true){
            ...
            $this->load->view('plantilla/header', $data);
            $this->load->view('plantilla/menu');
            $this->load->view('inv_almacen/v_inv_almacen_frm', $data);
            $this->load->view('plantilla/footer');                
        }else{
            $this->session->sess_destroy('username');
            $data['ruta_llamados_head'] = "plantilla/llamados_head/llamados_head_basicos.php";
    ...
    public function agregar_guardar(){
        ...
        if($logged == TRUE){            
            ...
            $valido = true;
            if($valido == true){
                    $id = $this->Inv_almacen_model->inv_almacen_insertar($nombre,$observacion);
                    if ($id > 0){
                        $mensaje = "Inserto la unidad medida con id ".$id;
                        $mensaje_2 = "Inserto la unidad medida ".$nombre;
                        $s_mensaje = array(
                                'inv_almacen_mensaje_tipo'         => 'alert-success',
                                'inv_almacen_mensaje_contenido'    => 'Registros insertado exitosamente, '.$mensaje_2
                        );
                        $this->session->set_userdata($s_mensaje);
                        $this->insertar_bitacora_2("inv_almacen", $mensaje, $mensaje_2);
                        redirect('inv_almacen/listar/cargo_ultima_pagina');
                    }else{
                        redirect('inv_almacen/agregar');
                    }                    
            }
        }else{
                    $this->session->sess_destroy('username');
    ...
    public function editar($id){
        ...
        if($logged == TRUE && $id_conf_roles_es_5 == true){
            ...
            $fila_reg = $this->Inv_almacen_model->inv_almacen_buscar_2($id);   //echo "<pre>"; print_r($fila_reg); echo "</pre>";
    ...
    public function editar_guardar(){
        ...
        if($logged == TRUE){
            ...
            $valido = true;
            if($valido == true){
                $oper_realizada = $this->Inv_almacen_model->inv_almacen_editar($id, $nombre, $observacion);
                ...
                if ($oper_realizada){
                    ...
                    $s_mensaje = array(
                            'inv_almacen_mensaje_tipo'  => 'alert-success',
                            'inv_almacen_mensaje_contenido'    => 'Registros actualizado exitosamente, '.$mensaje_2
                    );
                    $this->session->set_userdata($s_mensaje);
                    $this->insertar_bitacora_2("inv_almacen", $mensaje, $mensaje_2);
    ...
    public function eliminar($id){
            ...
            if($logged == TRUE && $id_conf_roles_es_5 == true){
                ...
                $matriz_almacen = $this->Inv_almacen_model->inv_almacen_buscar_2($id);
                $almacen_nombre = $matriz_almacen[0]->nombres;
                ...
                if( $valido == true ){
                    $oper_realizada = $this->Inv_almacen_model->inv_almacen_eliminar($id);
                    if ($oper_realizada){
                            $mensaje = "Eliminó el unidad medida con id ".$id;
                            $mensaje_2 = "Eliminó el unidad medida ".$almacen_nombre;
                            $s_mensaje = array(
                                    'inv_almacen_mensaje_tipo'         => 'alert-success',
                                    'inv_almacen_mensaje_contenido'    => 'Registros eliminado exitosamente, '.$mensaje_2
                            );
                            $this->session->set_userdata($s_mensaje);
                            $this->insertar_bitacora_2("inv_almacen", $mensaje, $mensaje_2);
                    }                
    ...
    public function get_inv_almacen_buscar_5(){
            $nombre     = $this->input->post('nombre');
            $id         = $this->input->post('id');
            $resultados     = $this->Inv_almacen_model->inv_almacen_buscar_5($nombre, $id); //echo "resultados *"; print_r($resultados); echo "*";
            echo json_encode($resultados);
    }

    public function get_inv_almacen_buscar_6(){
            $nombre     = $this->input->post('nombre');
            $resultados = $this->Inv_almacen_model->inv_almacen_buscar_6($nombre); //echo "resultados *"; print_r($resultados); echo "*";
            echo json_encode($resultados);
    }       
