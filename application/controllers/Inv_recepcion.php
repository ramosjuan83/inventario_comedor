...
class Inv_recepcion extends Controlador_padre {
    ...
    public function listar($memoria = 'false'){
        ...
        if($logged == TRUE && $id_conf_roles_es_5 == true){ 
                ...
                if($memoria == 'limpiar'){
                    $this->session->unset_userdata('inv_recepcion_b_texto');
                }
                
                if (	isset ($_POST['b_texto'])	){
                        $b_texto = $_POST['b_texto'];
                        $s_busquedad = array(
                                'inv_recepcion_b_texto' => $b_texto
                        );
                        $this->session->set_userdata($s_busquedad);
                }else{
                        $b_texto = "";
                        if( strlen($this->session->userdata('inv_recepcion_b_texto')) > 0  ){
                            $b_texto = $this->session->userdata('inv_recepcion_b_texto');
                        }
                }
                $b_texto = str_replace("'", "", $b_texto);
                
                if($memoria == 'cargo_ultima_pagina'){
                        $pagina_actual = $this->session->userdata('inv_recepcion_ultima_pagina');    
                        ...
                        $s_lista = array(
                            'inv_recepcion_ultima_pagina' => $pagina_actual
                        );
                ...
                $config['base_url'] = base_url().'index.php/inv_recepcion/listar/pagina'; // parametro base de la aplicación, si tenemos un .htaccess nos evitamos el index.php
                $total_rows = $this->Inv_recepcion_model->inv_recepcion_num_reg($b_texto);
                ...
                $dat_list = $this->Inv_recepcion_model->inv_recepcion_buscar($b_texto, $config['per_page'],$segmento);
//echo "<br>dat_list *<pre>"; print_r($dat_list); echo "</pre>*";                
                
                if($dat_list != false){
                    for($i = 0; $i < count($dat_list); $i++){
                        // $recepcion_id = $dat_list[$i]->id;  //echo "gerencias_id *".$gerencias_id."*";
                        // unset($matriz_comensales);                        
                        // $matriz_personal_ivic = $this->Personal_ivic_model->personal_ivic_buscar_9($recepcion_id);
...
//echo "<br>dat_list *<pre>"; print_r($dat_list); echo "</pre>*";
                ...
                $this->load->view('inv_recepcion/v_inv_recepcion_listar', $dat_list);
    ...
    public function agregar(){
        ...
        if($logged == TRUE && $id_conf_roles_es_5 == true){
            ...
            $this->load->view('plantilla/header', $data);
            $this->load->view('plantilla/menu');
            $this->load->view('inv_recepcion/v_inv_recepcion_frm', $data);
            $this->load->view('plantilla/footer');                
        }else{
            $this->session->sess_destroy('username');
            $data['ruta_llamados_head'] = "plantilla/llamados_head/llamados_head_basicos.php";
    ...
    public function agregar_guardar(){
        ...
        if($logged == TRUE){            
            ...
            $valor = "fechaAux";          if ( isset($_POST[$valor])              ){	$fechaAux = ucfirst( $_POST[$valor] );	}else{	$fechaAux	= "";	}
            ...
            $valor = "fecha_vencimiento";          if ( isset($_POST[$valor])              ){	$fecha_vencimiento = ucfirst( $_POST[$valor] );	}else{	$fecha_vencimiento	= "";	}
            ...
            if($valido == true){
                    $id = $this->Inv_recepcion_model->inv_recepcion_insertar($nombre,$id_articulo,$fecha,$id_almacen,$cantidad,$fecha_vencimiento);
                    if ($id > 0){
                        $mensaje = "Inserto la recepción con id ".$id;
                        $mensaje_2 = "Inserto la recepción ".$nombre;
                        $s_mensaje = array(
                                'inv_recepcion_mensaje_tipo'         => 'alert-success',
                                'inv_recepcion_mensaje_contenido'    => 'Registros insertado exitosamente, '.$mensaje_2
                        );
                        $this->session->set_userdata($s_mensaje);
                        $this->insertar_bitacora_2("inv_recepcion", $mensaje, $mensaje_2);
                        redirect('inv_recepcion/listar/cargo_ultima_pagina');
                    }else{
                        redirect('inv_recepcion/agregar');
                    }                    
            }else{

                $mensaje_2 = "La recepción ";
                $s_mensaje = array(
                    'inv_recepcion_mensaje_tipo'         => 'alert-danger',
                    'inv_recepcion_mensaje_contenido'    => 'La cantidad es superior a la disponibilidad , '." ".$disponible." ".$mensaje_2
                );
                $this->session->set_userdata($s_mensaje);
                redirect('Inv_recepcion/agregar');
    ...
    public function editar($id){
        ...
        if($logged == TRUE && $id_conf_roles_es_5 == true){
            ...
            $fila_reg = $this->Inv_recepcion_model->inv_recepcion_buscar_2($id);   //echo "<pre>"; print_r($fila_reg); echo "</pre>";
            ...
            if(strlen($fila_reg[0]->fecha_vencimiento) > 2){
                $fila_reg[0]->fecha_vencimiento = $this->ordena_fecha_4($fila_reg[0]->fecha_vencimiento);    
            }
    ...
    public function editar_guardar(){
        ...
        if($logged == TRUE){
            ...
            if(strlen($fecha_vencimiento) > 2){     $fecha_vencimiento = $this->ordena_fecha_3($fecha_vencimiento);    }   
            
            
            // echo "fecha_vencimiento".$fecha_vencimiento."fecha".$fecha;
            // phpinfo();
            ...
            if($valido == true){
                $oper_realizada = $this->Inv_recepcion_model->inv_recepcion_editar($id,$id_articulo,$fecha,$id_almacen,$cantidad,$nombre,$fecha_vencimiento);
                //echo "oper_realizada *".$oper_realizada."*";
                if ($oper_realizada){
                    $mensaje = "Actualizó la recepción con id ".$id;
                    $mensaje_2 = "Actualizó la recepción ".$nombre;
                    $s_mensaje = array(
                            'inv_recepcion_mensaje_tipo'  => 'alert-success',
                            'inv_recepcion_mensaje_contenido'    => 'Registros actualizado exitosamente, '.$mensaje_2
                    );
                    $this->session->set_userdata($s_mensaje);
                    $this->insertar_bitacora_2("inv_recepcion", $mensaje, $mensaje_2);
                ...
                $this->session->set_userdata($s_mensaje);
                redirect('Inv_recepcion/editar/'.$id);
    ...
    public function eliminar($id){
            ...
            if($logged == TRUE && $id_conf_roles_es_5 == true){
                ...
                $matriz_recepcion = $this->Inv_recepcion_model->inv_recepcion_buscar_2($id);
                $recepcion_nombre = $matriz_recepcion[0]->nombres;
                ...
                if( $valido == true ){
                    $oper_realizada = $this->Inv_recepcion_model->inv_recepcion_eliminar($id);
                    if ($oper_realizada){
                            $mensaje = "Eliminó el unidad medida con id ".$id;
                            $mensaje_2 = "Eliminó el unidad medida ".$recepcion_nombre;
                            $s_mensaje = array(
                                    'inv_recepcion_mensaje_tipo'         => 'alert-success',
                                    'inv_recepcion_mensaje_contenido'    => 'Registros eliminado exitosamente, '.$mensaje_2
                            );
                            $this->session->set_userdata($s_mensaje);
                            $this->insertar_bitacora_2("inv_recepcion", $mensaje, $mensaje_2);
                    }                
    ...
    public function ver_pdf_lista($b_texto){
        ...
        if($logged == TRUE){
            ...
            $matriz_recepcion = $this->Inv_recepcion_model->inv_recepcion_buscar($b_texto, "", ""); 

            $data['matriz_recepcion'] = $matriz_recepcion;   
            
            $this->load->view('inv_recepcion/reporte_pdf_lista', $data); 
        }        
    }  
    ...
    public function ver_excel_lista($b_texto){
        ...
        if($logged == TRUE){
            ...
            $this->load->view('inv_recepcion/reporte_excel_lista.php', $data);
    ...
    public function get_inv_recepcion_buscar_5(){
            $nombre     = $this->input->post('nombre');
            $id         = $this->input->post('id');
            $resultados     = $this->Inv_recepcion_model->inv_recepcion_buscar_5($nombre, $id); //echo "resultados *"; print_r($resultados); echo "*";
            echo json_encode($resultados);
    }

    public function get_inv_recepcion_buscar_6(){
            $nombre     = $this->input->post('nombre');
            $resultados = $this->Inv_recepcion_model->inv_recepcion_buscar_6($nombre); //echo "resultados *"; print_r($resultados); echo "*";
            echo json_encode($resultados);
    }  

    public function get_inv_recepcion_buscar_capacidad(){
        $almacen     = $this->input->post('almacen');
        $articulo     = $this->input->post('articulo');

        $resultados = $this->Inv_inventario_model->inv_inventario_capacidad($articulo,$almacen); //echo "resultados *"; print_r($resultados); echo "*";
        echo json_encode($resultados);
    } 
