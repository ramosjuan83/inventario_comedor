...
class Cargos extends Controlador_padre {
    ...
    public function listar($memoria = 'false'){
        ...
        if($logged == TRUE && $id_conf_roles_es_2 == true){ 
                ...
                $config['base_url'] = base_url().'index.php/cargos/listar/pagina'; // parametro base de la aplicación, si tenemos un .htaccess nos evitamos el index.php
                $total_rows = $this->Cargos_model->cargos_num_reg($b_texto);
...
//echo "<br>dat_list *<pre>"; print_r($dat_list); echo "</pre>*";
                ...
                $this->load->view('cargos/v_cargos_listar', $dat_list);
    ...
    public function agregar(){
        ...
        if($logged == TRUE && $id_conf_roles_es_2 == true){
            ...
            $this->load->view('plantilla/header', $data);
            $this->load->view('plantilla/menu');
            $this->load->view('cargos/v_cargos_frm', $data);
            $this->load->view('plantilla/footer');                
        }else{
            $this->session->sess_destroy('username');
            $data['ruta_llamados_head'] = "plantilla/llamados_head/llamados_head_basicos.php";
    ...
    public function agregar_guardar(){
        ...
        if($logged == TRUE){            
            ...
            //$valido = false;
            //$cargos_num_reg = $this->Cargos_model->cargos_num_reg_2($parametros['nombre']); //echo "cli_clientes_num_reg *".$cargos_num_reg."*";
            //if($cargos_num_reg == 0){ $valido = true; }
            //else{
            ...
            $valido = true;
            if($valido == true){
                    $id = $this->Cargos_model->cargos_insertar($nombre);
                    if ($id > 0){
                        $mensaje = "Inserto el cargo con id ".$id;
                        $mensaje_2 = "Inserto el cargo ".$nombre;
                        $s_mensaje = array(
                                'cargos_mensaje_tipo'         => 'alert-success',
                                'cargos_mensaje_contenido'    => 'Registros insertado exitosamente, '.$mensaje_2
                        );
                        $this->session->set_userdata($s_mensaje);
                        $this->insertar_bitacora_2("cargos", $mensaje, $mensaje_2);
                        redirect('cargos/listar/cargo_ultima_pagina');
                    }else{
                        redirect('cargos/agregar');
                    }                    
            }
        }else{
                    $this->session->sess_destroy('username');
    ...
    public function editar($id){
        ...
        if($logged == TRUE && $id_conf_roles_es_2 == true){
            ...
            $fila_reg = $this->Cargos_model->cargos_buscar_2($id);   //echo "<pre>"; print_r($fila_reg); echo "</pre>";
    ...
    public function editar_guardar(){
        ...
        if($logged == TRUE){
            ...
            $valido = true;
            if($valido == true){
                $oper_realizada = $this->Cargos_model->cargos_editar($id, $nombre);
                //echo "oper_realizada *".$oper_realizada."*";
                if ($oper_realizada){
                    $mensaje = "Actualizó el cargos con id ".$id;
                    $mensaje_2 = "Actualizó el cargo ".$nombre;
                    $s_mensaje = array(
                            'cargos_mensaje_tipo'  => 'alert-success',
                            'cargos_mensaje_contenido'    => 'Registros actualizado exitosamente, '.$mensaje_2
                    );
                    $this->session->set_userdata($s_mensaje);
                    $this->insertar_bitacora_2("cargos", $mensaje, $mensaje_2);
    ...
    public function eliminar($id){
            ...
            if($logged == TRUE && $id_conf_roles_es_2 == true){
                ...
                $matriz_cargos = $this->Cargos_model->cargos_buscar_2($id);
                $cargos_nombre = $matriz_cargos[0]->nombres;
                ...
                //    $mensaje_2 = "Error al eliminar, la Carrera ".$cargos_nombre." se encuentra asociado con un Pensum";
                //    $s_mensaje = array(
                //            'cargos_mensaje_tipo'         => 'alert-danger',
                //            'cargos_mensaje_contenido'    => 'El registro no fue eliminado, '.$mensaje_2
                //    );
                //    $this->session->set_userdata($s_mensaje);
                //    $this->insertar_bitacora_2("cargos", $mensaje, $mensaje_2);                    
                //}
                ...
                if( $valido == true ){
                    $oper_realizada = $this->Cargos_model->cargos_eliminar($id);
                    if ($oper_realizada){
                            $mensaje = "Eliminó el Cargo con id ".$id;
                            $mensaje_2 = "Eliminó el Cargo ".$cargos_nombre;
                            $s_mensaje = array(
                                    'cargos_mensaje_tipo'         => 'alert-success',
                                    'cargos_mensaje_contenido'    => 'Registros eliminado exitosamente, '.$mensaje_2
                            );
                            $this->session->set_userdata($s_mensaje);
                            $this->insertar_bitacora_2("cargos", $mensaje, $mensaje_2);
                    }                
    ...
    public function get_cargos_buscar_5(){
            $nombre     = $this->input->post('nombre');
            $id         = $this->input->post('id');
            $resultados     = $this->Cargos_model->cargos_buscar_5($nombre, $id); //echo "resultados *"; print_r($resultados); echo "*";
            echo json_encode($resultados);
    }

    public function get_cargos_buscar_6(){
            $nombre     = $this->input->post('nombre');
            $resultados = $this->Cargos_model->cargos_buscar_6($nombre); //echo "resultados *"; print_r($resultados); echo "*";
            echo json_encode($resultados);
    }      
