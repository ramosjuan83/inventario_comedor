...
class Inv_tipo_articulo extends Controlador_padre {
    ...
    public function listar($memoria = 'false'){
        ...
        if($logged == TRUE && $id_conf_roles_es_5 == true){ 
                ...
                if($memoria == 'limpiar'){
                    $this->session->unset_userdata('inv_tipo_articulo_b_texto');
                }
                
                if (	isset ($_POST['b_texto'])	){
                        $b_texto = $_POST['b_texto'];
                        $s_busquedad = array(
                                'inv_tipo_articulo_b_texto' => $b_texto
                        );
                        $this->session->set_userdata($s_busquedad);
                }else{
                        $b_texto = "";
                        if( strlen($this->session->userdata('inv_tipo_articulo_b_texto')) > 0  ){
                            $b_texto = $this->session->userdata('inv_tipo_articulo_b_texto');
                        }
                }
                $b_texto = str_replace("'", "", $b_texto);
                
                if($memoria == 'cargo_ultima_pagina'){
                        $pagina_actual = $this->session->userdata('inv_tipo_articulo_ultima_pagina');    
                        ...
                        $s_lista = array(
                            'inv_tipo_articulo_ultima_pagina' => $pagina_actual
                        );
                ...
                $config['base_url'] = base_url().'index.php/inv_tipo_articulo/listar/pagina'; // parametro base de la aplicación, si tenemos un .htaccess nos evitamos el index.php
                $total_rows = $this->Inv_tipo_articulo_model->inv_tipo_articulo_num_reg($b_texto);
                ...
                $dat_list = $this->Inv_tipo_articulo_model->inv_tipo_articulo_buscar($b_texto, $config['per_page'],$segmento);
//echo "<br>dat_list *<pre>"; print_r($dat_list); echo "</pre>*";                
                
                if($dat_list != false){
                    for($i = 0; $i < count($dat_list); $i++){
                        // $tipo_articulo_id = $dat_list[$i]->id;  //echo "gerencias_id *".$gerencias_id."*";
                        // unset($matriz_comensales);                        
                        // $matriz_personal_ivic = $this->Personal_ivic_model->personal_ivic_buscar_9($tipo_articulo_id);
...
//echo "<br>dat_list *<pre>"; print_r($dat_list); echo "</pre>*";
                ...
                $this->load->view('inv_tipo_articulo/v_inv_tipo_articulo_listar', $dat_list);
    ...
    public function agregar(){
        ...
        if($logged == TRUE && $id_conf_roles_es_5 == true){
            ...
            $this->load->view('plantilla/header', $data);
            $this->load->view('plantilla/menu');
            $this->load->view('inv_tipo_articulo/v_inv_tipo_articulo_frm', $data);
            $this->load->view('plantilla/footer');                
        }else{
            $this->session->sess_destroy('username');
            $data['ruta_llamados_head'] = "plantilla/llamados_head/llamados_head_basicos2.php";
    ...
    public function agregar_guardar(){
        ...
        if($logged == TRUE){            
            ...
            $valor = "observacion";          if ( isset($_POST[$valor])              ){	$observacion = ucfirst( $_POST[$valor] );	}else{	$nombre	= "";	}
            ...
            $valido = true;
            if($valido == true){
                    $id = $this->Inv_tipo_articulo_model->inv_tipo_articulo_insertar($nombre,$observacion);
                    if ($id > 0){
                        $mensaje = "Inserto la unidad medida con id ".$id;
                        $mensaje_2 = "Inserto la unidad medida ".$nombre;
                        $s_mensaje = array(
                                'inv_tipo_articulo_mensaje_tipo'         => 'alert-success',
                                'inv_tipo_articulo_mensaje_contenido'    => 'Registros insertado exitosamente, '.$mensaje_2
                        );
                        $this->session->set_userdata($s_mensaje);
                        $this->insertar_bitacora_2("inv_tipo_articulo", $mensaje, $mensaje_2);
                        redirect('inv_tipo_articulo/listar/cargo_ultima_pagina');
                    }else{
                        redirect('inv_tipo_articulo/agregar');
                    }                    
            }
        }else{
                    $this->session->sess_destroy('username');
    ...
    public function editar($id){
        ...
        if($logged == TRUE && $id_conf_roles_es_5 == true){
            ...
            $fila_reg = $this->Inv_tipo_articulo_model->inv_tipo_articulo_buscar_2($id);   //echo "<pre>"; print_r($fila_reg); echo "</pre>";
    ...
    public function editar_guardar(){
        ...
        if($logged == TRUE){
            ...
            $valido = true;
            if($valido == true){
                $oper_realizada = $this->Inv_tipo_articulo_model->inv_tipo_articulo_editar($id, $nombre, $observacion);
                ...
                if ($oper_realizada){
                    ...
                    $s_mensaje = array(
                            'inv_tipo_articulo_mensaje_tipo'  => 'alert-success',
                            'inv_tipo_articulo_mensaje_contenido'    => 'Registros actualizado exitosamente, '.$mensaje_2
                    );
                    $this->session->set_userdata($s_mensaje);
                    $this->insertar_bitacora_2("inv_tipo_articulo", $mensaje, $mensaje_2);
                    ...
                    redirect('inv_tipo_articulo/editar');
                }
            }
        }else{
            $this->session->sess_destroy('username');
    ...
    public function eliminar($id){
            ...
            if($logged == TRUE && $id_conf_roles_es_5 == true){
                ...
                $matriz_tipo_articulo = $this->Inv_tipo_articulo_model->inv_tipo_articulo_buscar_2($id);
                $tipo_articulo_nombre = $matriz_tipo_articulo[0]->nombres;
                ...
                if( $valido == true ){
                    $oper_realizada = $this->Inv_tipo_articulo_model->inv_tipo_articulo_eliminar($id);
                    if ($oper_realizada){
                            $mensaje = "Eliminó el tipo de artículo con id ".$id;
                            $mensaje_2 = "Eliminó el tipo de artículo ".$tipo_articulo_nombre;
                            $s_mensaje = array(
                                    'inv_tipo_articulo_mensaje_tipo'         => 'alert-success',
                                    'inv_tipo_articulo_mensaje_contenido'    => 'Registros eliminado exitosamente, '.$mensaje_2
                            );
                            $this->session->set_userdata($s_mensaje);
                            $this->insertar_bitacora_2("inv_tipo_articulo", $mensaje, $mensaje_2);
                    }                
    ...
    public function get_inv_tipo_articulo_buscar_5(){
            $nombre     = $this->input->post('nombre');
            $id         = $this->input->post('id');
            $resultados     = $this->Inv_tipo_articulo_model->inv_tipo_articulo_buscar_5($nombre, $id); //echo "resultados *"; print_r($resultados); echo "*";
            echo json_encode($resultados);
    }

    public function get_inv_tipo_articulo_buscar_6(){
            $nombre     = $this->input->post('nombre');
            $resultados = $this->Inv_tipo_articulo_model->inv_tipo_articulo_buscar_6($nombre); //echo "resultados *"; print_r($resultados); echo "*";
            echo json_encode($resultados);
    }       
