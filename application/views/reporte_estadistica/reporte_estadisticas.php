...
class PDF extends PDF_MC_Table
{
	...
	function Header()
	{
		...
		$this->Image(base_url($imagen1_ruta), $PosicionX, $PosicionY, $anchoFoto, $alto); // IMPRIME LA FOTO
                ...
                //$this->Cell(50, 4, $this->ajuste_texto("Despacho"), 0, 0, 'L', true);                
                $this->Cell(60, 4, $this->ajuste_texto("Artículo"), 0, 0, 'C', true);
                $this->Cell(60, 4, $this->ajuste_texto("Capacidad de Almacén"), 0, 0, 'C', true);
                $this->Cell(60, 4, $this->ajuste_texto("Disponibilidad"), 0, 0, 'C', true);
                $this->Cell(60, 4, $this->ajuste_texto("Merma"), 0, 0, 'C', true);
                $this->Ln();
	}
...
if($matriz_estadisticas != false){
    $registros_totales = count($matriz_estadisticas);
}
...
if($matriz_estadisticas != false){
    for($i = 0; $i < count($matriz_estadisticas); $i++){
            ...
            //$i_mas_1 = $i + 1;
            $nombre_articulo=$matriz_estadisticas[$i]->nombre_articulo;
            $nombre_almacen=$matriz_estadisticas[$i]->nombre_almacen;
            $capacidad_almacen=$matriz_estadisticas[$i]->capacidad_almacen;
            $disponible=$matriz_estadisticas[$i]->disponible;
            $merma=$matriz_estadisticas[$i]->merma;
            $unidad_medida=$matriz_estadisticas[$i]->nombre_unidad;
            $rowspan=$matriz_estadisticas[$i]->rowspan;

            
            // $pdf->Cell(50, 4,$pdf->ajuste_texto($rowspan==1?$nombre_almacen:''), 0, 0, 'L', true);                
            // $pdf->Cell(50, 4, $pdf->ajuste_texto($nombre_articulo), 0, 0, 'C', true);
            // $pdf->Cell(50, 4, $pdf->ajuste_texto(number_format($capacidad_almacen,"2",",",".")." (".$unidad_medida.")"), 0, 0, 'L', true);
            // $pdf->Cell(50, 4, $pdf->ajuste_texto(number_format($disponible,"2",",",".")." (".$unidad_medida.")"), 0, 0, 'C', true);
            // $pdf->Cell(50, 4, $pdf->ajuste_texto(number_format($merma,"2",",",".")." (".$unidad_medida.")"), 0, 0, 'C', true);
            ...
            $pdf->Cell(60, 4, $pdf->ajuste_texto($nombre_articulo), 1, 0, 'L', true);
            $pdf->Cell(60, 4, $pdf->ajuste_texto(number_format($capacidad_almacen,"2",",",".")." (".$unidad_medida.")"), 1, 0, 'R', true);
            $pdf->Cell(60, 4, $pdf->ajuste_texto(number_format($disponible,"2",",",".")." (".$unidad_medida.")"), 1, 0, 'R', true);
            $pdf->Cell(60, 4, $pdf->ajuste_texto(number_format($merma,"2",",",".")." (".$unidad_medida.")"), 1, 1, 'R', true);
            


            // $pdf->Row(array(
            //         //$id
            //     //     $pdf->ajuste_texto($rowspan==1?$nombre_almacen:'')
            //         $pdf->ajuste_texto($nombre_articulo)
            //     ,   $pdf->ajuste_texto(number_format($capacidad_almacen,"2",",",".")." (".$unidad_medida.")")
            //     ,   $pdf->ajuste_texto(number_format($disponible,"2",",",".")." (".$unidad_medida.")")
            //     ,   $pdf->ajuste_texto(number_format($merma,"2",",",".")." (".$unidad_medida.")")
            // ));
            $pdf->Ln(0);
    }
}
...
$pdf->Output();
$nombre = "reporte_estadisticas.pdf";
