...
if($matriz_personal_visitante != false){
    $registros_totales = count($matriz_personal_visitante);
}
...
//IMPRIME LOS TITULOS
$fila = 1;
$sheet->setCellValue("A".$fila, ajuste_texto("Personal Externo"));
$hoja->getStyle('A'.$fila.':E'.$fila)->getFont()->setBold(true)->setSize(12);
$fila++;

$titulo = "Registros Totales ".$registros_totales;
$sheet->setCellValue("A".$fila, ajuste_texto($titulo));
$hoja->getStyle('A'.$fila.':E'.$fila)->getFont()->setBold(false)->setSize(12);
$fila++;
...
$sheet->setCellValue("B".$fila, "NOMBRES");
$sheet->setCellValue("C".$fila, "APELLIDOS");
$sheet->setCellValue("D".$fila, "ESTADO");

$hoja->getStyle('A'.$fila.':D'.$fila)->getFont()->setBold(false)->setSize(12);
...
$hoja->getStyle('A'.$fila.':D'.$fila)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('1F74AF');
$hoja->getStyle('A'.$fila.':D'.$fila)->getFont()->getColor()->setRGB('FFFFFF');
$fila++;
...
if($matriz_personal_visitante != false){
    for($i = 0; $i < count($matriz_personal_visitante); $i++){
        $sheet->setCellValue("A".$fila, ajuste_texto($matriz_personal_visitante[$i]->cedula));
        $sheet->setCellValue("B".$fila, ajuste_texto($matriz_personal_visitante[$i]->nombres));
        $sheet->setCellValue("C".$fila, ajuste_texto($matriz_personal_visitante[$i]->apellidos));
        $sheet->setCellValue("D".$fila, ajuste_texto($matriz_personal_visitante[$i]->estado_nombre));

        $hoja->getStyle('A'.$fila.':D'.$fila)->getFont()->setBold(false)->setSize(12);
        $hoja->getStyle('A'.$fila.':D'.$fila)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('FFFBFE');
        $hoja->getStyle('A'.$fila)->getAlignment()->setHorizontal('center');        
        $hoja->getStyle('B'.$fila)->getAlignment()->setHorizontal('left');        
        $hoja->getStyle('C'.$fila)->getAlignment()->setHorizontal('left');        
        $hoja->getStyle('D'.$fila)->getAlignment()->setHorizontal('center');        
...
$fileName="Personal_Visitante.xlsx";
# Crear un "escritor"
