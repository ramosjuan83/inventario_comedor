...
if($matriz_personal_visitante != false){
    for($i = 0; $i < count($matriz_personal_visitante); $i++){
            ...
            //$i_mas_1 = $i + 1;
/*            
            $id          = $matriz_personal_visitante[$i]->comensales_id;
            $fecha_hora  = $matriz_personal_visitante[$i]->comensales_fecha." ".$matriz_personal_visitante[$i]->comensales_hora;
            
            //echo "matriz_comensales <br />*<pre>"; print_r($matriz_personal_visitante); echo "</pre>*";
            
            $personal = $matriz_personal_visitante[$i]->personal;
            
            $tipo_personal = "";
            if($matriz_personal_visitante[$i]->comensales_id_personal_ivic > 0){
                    $tipo_personal = "IVIC";
            }
            if($matriz_personal_visitante[$i]->comensales_id_personal_visitante > 0){
                    $tipo_personal = "EXTERNO";
            }            

            $comida_tipo = $matriz_personal_visitante[$i]->comedor_comida_tipo_nombre;
            
            $personal_visitante_tipo_nombre = $matriz_personal_visitante[$i]->personal_visitante_tipo_nombre;
*/
        
            $personal = $matriz_personal_visitante[$i]->nombres." ".$matriz_personal_visitante[$i]->apellidos;       
        
            $pdf->SetWidths(array(20,136,40));
            $pdf->SetAligns(array('C','L','C'));
            $pdf->Row(array(
                    //$id
                    //$pdf->ajuste_texto($matriz_personal_visitante[$i]->carnet_codigo)
                    $pdf->ajuste_texto($matriz_personal_visitante[$i]->cedula)
                ,   $pdf->ajuste_texto($personal)
                ,   $pdf->ajuste_texto($matriz_personal_visitante[$i]->estado_nombre)
            ));
            $pdf->Ln(0);
    }
}
