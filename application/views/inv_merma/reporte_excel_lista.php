...
$registros_totales = 0;
if($matriz_merma != false){
    $registros_totales = count($matriz_merma);
}
...
//IMPRIME LOS TITULOS
$fila = 1;
$sheet->setCellValue("A".$fila, ajuste_texto("Merma"));
...
$sheet->setCellValue("B".$fila, "FECHA MERMA");
...
if($matriz_merma != false){
    for($i = 0; $i < count($matriz_merma); $i++){
        $sheet->setCellValue("A".$fila, ajuste_texto($matriz_merma[$i]->nombre_articulo));
        $sheet->setCellValue("B".$fila, ajuste_texto($matriz_merma[$i]->fecha_merma_2));
        $sheet->setCellValue("C".$fila, ajuste_texto($matriz_merma[$i]->nombre_almacen));
        $sheet->setCellValue("D".$fila, ajuste_texto($matriz_merma[$i]->cantidad));
        $sheet->setCellValue("E".$fila, ajuste_texto($matriz_merma[$i]->observacion));
