...
class PDF extends PDF_MC_Table
{
	...
	function Header()
	{
		...
		$this->Image(base_url($imagen1_ruta), $PosicionX, $PosicionY, $anchoFoto, $alto); // IMPRIME LA FOTO
                ...
                $this->Cell(30, 4, $this->ajuste_texto("Fecha Merma"), 0, 0, 'C', true);
...
if($matriz_merma != false){
    for($i = 0; $i < count($matriz_merma); $i++){
            ...
            $pdf->Row(array(
                    //$id
                    //$pdf->ajuste_texto($matriz_merma[$i]->carnet_codigo)
                    $pdf->ajuste_texto($matriz_merma[$i]->nombre_articulo)
                ,   $pdf->ajuste_texto($matriz_merma[$i]->fecha_merma_2)
                ,   $pdf->ajuste_texto($matriz_merma[$i]->nombre_almacen)
                ,   $pdf->ajuste_texto($matriz_merma[$i]->cantidad)
                ,   $pdf->ajuste_texto($matriz_merma[$i]->observacion)
            ));
            $pdf->Ln(0);
    }
}
