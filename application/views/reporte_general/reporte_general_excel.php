...
//IMPRIME LOS TITULOS
$fila = 1;
$sheet->setCellValue("A".$fila, ajuste_texto("Generales"));
...
// $sheet->setCellValue("A".$fila, "Nombre de artículo");
// $sheet->setCellValue("B".$fila, "Despachado");
// $sheet->setCellValue("C".$fila, "Recibido");
// $sheet->setCellValue("D".$fila, "Disponibilidad");
// $sheet->setCellValue("E".$fila, "Despachado Traslado");
// $sheet->setCellValue("F".$fila, "Recibido Traslado");
// $sheet->setCellValue("G".$fila, "Merma");
...
if($matriz_generales != false){
    for($i = 0; $i < count($matriz_generales); $i++){
        ...
        if($rowspan==1){
            ...
            $sheet->setCellValue("D".$fila, ajuste_texto($nombre_almacen));
            $sheet->getStyle('D'.$fila)->getFont()->setBold(true)->setSize(12);
            $fila++;
        ...
        $sheet->setCellValue("A".$fila, ajuste_texto($nombre_articulo));
        $sheet->setCellValue("B".$fila, ajuste_texto($cantidad_despacho));
        $sheet->setCellValue("C".$fila, ajuste_texto($cantidad_recepcion));
        $sheet->setCellValue("D".$fila, ajuste_texto($disponible));
        $sheet->setCellValue("E".$fila, ajuste_texto($cantidad_despacho_traslado));
        $sheet->setCellValue("F".$fila, ajuste_texto($cantidad_recepcion_traslado));
        $sheet->setCellValue("G".$fila, ajuste_texto($cantidad_merma));
...
$fileName="reporte_generales.xlsx";
# Crear un "escritor"
