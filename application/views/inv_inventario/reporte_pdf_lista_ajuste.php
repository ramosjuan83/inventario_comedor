...
class PDF extends PDF_MC_Table
{
	...
	function Header()
	{
		...
		$this->Image(base_url($imagen1_ruta), $PosicionX, $PosicionY, $anchoFoto, $alto); // IMPRIME LA FOTO
                ...
                $this->SetDrawColor(210,210,210);
                $this->SetTextColor(57, 62, 83);  // Establece el color del texto                 
                $this->Cell(196, 19, $this->titulo, 1, 1, 'C');
                $this->Ln(4);  
                ...
                $this->SetDrawColor(210,210,210);
                $this->SetTextColor(57, 62, 83);  // Establece el color del texto                 
                $this->Cell(196, 12, "EMPRESA: INSTITUTO VENEZOLANO DE INVESTIGACIONES CIENTIFICAS", 1, 0, 'L');
                $this->Ln(4); 
                $this->SetFont('Arial','B',8);
                $this->Cell(196, 11, "ALMACEN: ".$this->ajuste_texto($this->almacen), 0, 1, 'L');
                //$this->Ln(4); 
                ...
                $this->SetFillColor(192, 192, 192);  // Establece el color del fondo de la celda                
                $this->SetFont('Arial','B',8);
                //$this->Cell(8, 4, $this->ajuste_texto("ID"), 0, 0, 'L', true);
                
                $this->Cell(80, 4, $this->ajuste_texto("Artículo"), 0, 0, 'C', true);
                $this->Cell(40, 4, $this->ajuste_texto("Disponible"), 0, 0, 'C', true);
                $this->Cell(40, 4, $this->ajuste_texto("Unidad"), 0, 0, 'C', true);
                $this->Cell(40, 4, $this->ajuste_texto("Cantidad"), 0, 0, 'C', true);
                $this->Ln();
	}
...
if($matriz_inventarios != false){
    for($i = 0; $i < count($matriz_inventarios); $i++){
            ...
            $pdf->SetWidths(array(80,40,40,40,40));
...
//$pdf->Output();
$nombre = "reporte_inventario_ajuste.pdf";
