...
//IMPRIME LOS TITULOS
$fila = 1;
$sheet->setCellValue("A".$fila, ajuste_texto("Inventario"));
...
$sheet->setCellValue("B".$fila, "ALMACËN");
$sheet->setCellValue("C".$fila, "CAPACIDAD  ALMACËN");
$sheet->setCellValue("D".$fila, "DISPONIBLE");
...
if($matriz_inventarios != false){
    for($i = 0; $i < count($matriz_inventarios); $i++){
        $sheet->setCellValue("A".$fila, ajuste_texto($matriz_inventarios[$i]->nombre_articulo));
        $sheet->setCellValue("B".$fila, ajuste_texto($matriz_inventarios[$i]->nombre_almacen));
        $sheet->setCellValue("C".$fila, ajuste_texto($matriz_inventarios[$i]->capacidad_almacen));
        $sheet->setCellValue("D".$fila, ajuste_texto($matriz_inventarios[$i]->disponible));
        $sheet->setCellValue("E".$fila, ajuste_texto($matriz_inventarios[$i]->nombre_medida));
