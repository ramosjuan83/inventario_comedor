...
//IMPRIME LOS TITULOS
$fila = 1;
$sheet->setCellValue("A".$fila, ajuste_texto("Traslado"));
...
$sheet->setCellValue("B".$fila, "FECHA TRASLADO");
$sheet->setCellValue("C".$fila, "ALMACËN ORIGEN");
$sheet->setCellValue("D".$fila, "ALMACËN DESTINO");
...
if($matriz_traslado != false){
    for($i = 0; $i < count($matriz_traslado); $i++){
        $sheet->setCellValue("A".$fila, ajuste_texto($matriz_traslado[$i]->nombre_articulo));
        $sheet->setCellValue("B".$fila, ajuste_texto($matriz_traslado[$i]->fecha_traslado_2));
        $sheet->setCellValue("C".$fila, ajuste_texto($matriz_traslado[$i]->nombre_almacen_origen));
        $sheet->setCellValue("D".$fila, ajuste_texto($matriz_traslado[$i]->nombre_almacen_origen));
        $sheet->setCellValue("E".$fila, ajuste_texto($matriz_traslado[$i]->cantidad));
        $sheet->setCellValue("F".$fila, ajuste_texto($matriz_traslado[$i]->observacion));
