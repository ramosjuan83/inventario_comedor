...
class PDF extends PDF_MC_Table
{
	...
	function Header()
	{
		...
		$this->Image(base_url($imagen1_ruta), $PosicionX, $PosicionY, $anchoFoto, $alto); // IMPRIME LA FOTO
                ...
                $this->Cell(196, 19, $this->ajuste_texto($this->titulo), 1, 0, 'C');
                $this->Ln(4);  
                ...
                $this->Cell(40, 4, $this->ajuste_texto("Artículo"), 0, 0, 'C', true);
                $this->Cell(30, 4, $this->ajuste_texto("Fecha Traslado"), 0, 0, 'C', true);
                $this->Cell(35, 4, $this->ajuste_texto("Almacén Origen"), 0, 0, 'C', true);
                $this->Cell(35, 4, $this->ajuste_texto("Almacén Destino"), 0, 0, 'C', true);
                $this->Cell(20, 4, $this->ajuste_texto("Cantidad"), 0, 0, 'C', true);
                $this->Cell(40, 4, $this->ajuste_texto("Observación"), 0, 0, 'C', true);
                $this->Ln();
	}
...
//}
$registros_totales = 0;
if($matriz_traslado != false){
    $registros_totales = count($matriz_traslado);
}
...
if($matriz_traslado != false){
    for($i = 0; $i < count($matriz_traslado); $i++){
        
            $pdf->SetDrawColor(210,210,210);
            $pdf->SetTextColor(57, 62, 83);  // Establece el color del texto            
            //$i_mas_1 = $i + 1;  
        
            $pdf->SetWidths(array(40,30,35,35,20,40));
            $pdf->SetAligns(array('L','C','L','L','C','L'));
            $pdf->Row(array(
                    //$id
                    //$pdf->ajuste_texto($matriz_traslado[$i]->carnet_codigo)
                    $pdf->ajuste_texto($matriz_traslado[$i]->nombre_articulo)
                ,   $pdf->ajuste_texto($matriz_traslado[$i]->fecha_traslado_2)
                ,   $pdf->ajuste_texto($matriz_traslado[$i]->nombre_almacen_origen)
                ,   $pdf->ajuste_texto($matriz_traslado[$i]->nombre_almacen_destino)
                ,   $pdf->ajuste_texto($matriz_traslado[$i]->cantidad)
                ,   $pdf->ajuste_texto($matriz_traslado[$i]->observacion)
            ));
            $pdf->Ln(0);
    }
}
