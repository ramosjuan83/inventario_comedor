...
class PDF extends PDF_MC_Table
{
	...
	function Header()
	{
		...
		$this->Image(base_url($imagen1_ruta), $PosicionX, $PosicionY, $anchoFoto, $alto); // IMPRIME LA FOTO
                ...
                $this->Cell(30, 4, $this->ajuste_texto("Fecha Egreso"), 0, 0, 'C', true);
                $this->Cell(40, 4, $this->ajuste_texto("Almacén"), 0, 0, 'C', true);
                $this->Cell(20, 4, $this->ajuste_texto("Cantidad"), 0, 0, 'C', true);
                $this->Cell(50, 4, $this->ajuste_texto("Observación"), 0, 0, 'C', true);
                $this->Ln();
	}
...
if($matriz_despacho != false){
    for($i = 0; $i < count($matriz_despacho); $i++){
            ...
            $pdf->SetWidths(array(40,30,40,20,50));
            $pdf->SetAligns(array('L','C','L','C','L'));
            $pdf->Row(array(
                    //$id
                    //$pdf->ajuste_texto($matriz_despacho[$i]->carnet_codigo)
                    $pdf->ajuste_texto($matriz_despacho[$i]->nombre_articulo)
                ,   $pdf->ajuste_texto($matriz_despacho[$i]->fecha_egreso_2)
                ,   $pdf->ajuste_texto($matriz_despacho[$i]->nombre_almacen)
                ,   $pdf->ajuste_texto($matriz_despacho[$i]->cantidad)
                ,   $pdf->ajuste_texto($matriz_despacho[$i]->observacion)
            ));
            $pdf->Ln(0);
    }
}
