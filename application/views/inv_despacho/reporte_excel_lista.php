...
$registros_totales = 0;
if($matriz_despacho != false){
    $registros_totales = count($matriz_despacho);
}
...
//IMPRIME LOS TITULOS
$fila = 1;
$sheet->setCellValue("A".$fila, ajuste_texto("Despacho"));
...
$sheet->setCellValue("B".$fila, "FECHA EGRESO");
...
$hoja->getStyle('A'.$fila.':E'.$fila)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('1F74AF');
$hoja->getStyle('A'.$fila.':E'.$fila)->getFont()->getColor()->setRGB('FFFFFF');
$fila++;
...
if($matriz_despacho != false){
    for($i = 0; $i < count($matriz_despacho); $i++){
        $sheet->setCellValue("A".$fila, ajuste_texto($matriz_despacho[$i]->nombre_articulo));
        $sheet->setCellValue("B".$fila, ajuste_texto($matriz_despacho[$i]->fecha_egreso_2));
        $sheet->setCellValue("C".$fila, ajuste_texto($matriz_despacho[$i]->nombre_almacen));
        $sheet->setCellValue("D".$fila, ajuste_texto($matriz_despacho[$i]->cantidad));
        $sheet->setCellValue("E".$fila, ajuste_texto($matriz_despacho[$i]->observacion));


        $hoja->getStyle('A'.$fila.':E'.$fila)->getFont()->setBold(false)->setSize(12);
        $hoja->getStyle('A'.$fila.':E'.$fila)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('FFFBFE');
