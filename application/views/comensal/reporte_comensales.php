...
if($matriz_comensales != false){
    for($i = 0; $i < count($matriz_comensales); $i++){
            ...
            /*
                ...
                $this->Cell(47, 4, $this->ajuste_texto("Estado Temporal"), 0, 0, 'C', true);
            */
            $pdf->SetWidths(array(30,18,75,30,30,30,47));
            $pdf->SetAligns(array('L','C','L','C','C','C','L'));
            $pdf->Row(array(
                    //$id
                    $pdf->ajuste_texto($fecha_hora)
                ,   $pdf->ajuste_texto($personal_cedula)
                ,   $pdf->ajuste_texto($personal)
                ,   $pdf->ajuste_texto($comida_tipo)
                ,   $tipo_personal
                ,   $pdf->ajuste_texto($personal_visitante_tipo_nombre)
                ,   $pdf->ajuste_texto($matriz_comensales[$i]->estado_temporal)
            ));
            $pdf->Ln(0);
    }
}
