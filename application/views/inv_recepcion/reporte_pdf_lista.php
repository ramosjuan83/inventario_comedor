...
class PDF extends PDF_MC_Table
{
	...
	function Header()
	{
		...
		$this->Image(base_url($imagen1_ruta), $PosicionX, $PosicionY, $anchoFoto, $alto); // IMPRIME LA FOTO
                ...
                $this->Cell(30, 4, $this->ajuste_texto("Artículo"), 0, 0, 'C', true);
                $this->Cell(30, 4, $this->ajuste_texto("Fecha Ingreso"), 0, 0, 'C', true);
                $this->Cell(40, 4, $this->ajuste_texto("Almacén"), 0, 0, 'C', true);
                $this->Cell(20, 4, $this->ajuste_texto("Cantidad"), 0, 0, 'C', true);
                $this->Cell(40, 4, $this->ajuste_texto("Fecha Vencimiento"), 0, 0, 'C', true);
...
if($matriz_recepcion != false){
    for($i = 0; $i < count($matriz_recepcion); $i++){
            ...
            $pdf->SetWidths(array(30,30,40,20,40,40));
            $pdf->SetAligns(array('L','C','L','C','C','L'));
            $pdf->Row(array(
                    //$id
                    //$pdf->ajuste_texto($matriz_recepcion[$i]->carnet_codigo)
                    $pdf->ajuste_texto($matriz_recepcion[$i]->nombre_articulo)
                ,   $pdf->ajuste_texto($matriz_recepcion[$i]->fecha_ingreso_2)
                ,   $pdf->ajuste_texto($matriz_recepcion[$i]->nombre_almacen)
                ,   $pdf->ajuste_texto($matriz_recepcion[$i]->cantidad)
                ,   $pdf->ajuste_texto($matriz_recepcion[$i]->fecha_vencimiento)
                ,   $pdf->ajuste_texto($matriz_recepcion[$i]->observacion)
            ));
            $pdf->Ln(0);
    }
}
